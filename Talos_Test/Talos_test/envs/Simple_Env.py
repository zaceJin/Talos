...
class Simple_Env(gym.Env):
    ...
    def step(self, action):
        ...
        basePositionPrevious = self.talos.getBasePosition()
        print(basePositionPrevious[2])
        # Unnormalized action
        ...
        basePosition = self.talos.getBasePosition()
        ...
        if self.index_step >= self.HORIZON:
            self.done = True
        # Get reward
        reward = self.getReward(jointPositionsPrevious, jointVelocitiesPrevious, jointPositions, jointVelocities,
                                basePositionPrevious, basePosition, action)
    ...
    def reset(self):
        ...
        p.resetBasePositionAndOrientation(self.talos.robotId, self.talos.robotStartPosition,
                                          self.talos.robotStartOrientation, self.client)
    ...
    def getReward(self, jointPositionsPrevious, jointVelocitiesPrevious, jointPositions, jointVelocities, basePositionPrevious, basePosition,action):
        reward = 0
        # Set a target for left arm
        jointsIndices = [11,12,13,14,15,16,17, # Left arm  4-10
        ...
        for i in jointsIndices:
            jointsTarget.append(
                self.talos.mapJointLimitAll[i][0])  # Target is the min value of this joint for this exercise
        # Get pos of left arm only
        aimJointPos, aimJointVel = [], []
        aimJointPosPrevious, aimJointVelPrevious = [], []
        for i in jointsIndices:
            index = self.talos.mapJointsToControlled[i]  # Index of the joints in the list of controlled joints
            # pos
            aimJointPos.append(jointPositions[index])
            aimJointPosPrevious.append(jointPositionsPrevious[index])
            # vel
            aimJointVel.append(jointVelocities[0])
            aimJointVelPrevious.append(jointVelocitiesPrevious[index])
        # (1) Punish for being far from the target
        distance = np.linalg.norm(np.array(1.08) - np.array(basePosition[2])) #/ math.sqrt(len(jointsIndices))
        rPos = -distance
        #print(rPos)
        ...
        distance = np.linalg.norm(np.array(aimJointPosPrevious) - np.array(aimJointPos)) / math.sqrt(
            len(jointsIndices))  # Value between [0,1]
        rEffort = -distance
        #print(rEffort)
        # Sum of rewards
    ...
    def isJointsInBounds(self, joint_positions, controlledJoints, bound_joints):
        ...
        for i in range(len(controlledJoints)):
            if (joint_positions[controlledJoints[i]] >= bound_joints[i][0] and joint_positions[controlledJoints[i]] <=
                    bound_joints[i][1]):
                counter_joints += 1
    ...
    def calculate_Torques(self, action):
        ...
        for i in range(len(action)):
            qdes_i = action[i] * (self.talos.mapJointLimit[i][1] - self.talos.mapJointLimit[i][0]) + \
                     self.talos.mapJointLimit[i][0]
